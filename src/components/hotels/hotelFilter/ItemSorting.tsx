...
function ItemSorting({ propertyCount, label }: SearchResultsHeaderProps) {
  ...
  const [selectedSort, setSelectedSort] = useState<string>(() =>
    query.has("orderby") ? query.get("orderby") || "" : sortOptions[0].value
  );
          ...
          <Button
            ...
            className={`${
              selectedSort === option.value
                ? "bg-gray-100 border-primary text-gray-900" // Active tab styles
                : "border-gray-300 text-gray-600 hover:bg-gray-50" // Default tab styles
            }`}
            onClick={() => handleSortChange(option)}
          >
            {option.label}
